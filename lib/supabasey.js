...
* @param {Object} [options.retry] Raw options passed to p-retry / node-retry
*
...
supabasey.init = function supabaseyInit(options) {
	...
	return Promise.resolve()
		...
		.then(async ()=> { // Login {{{
			if (!settings.login) { // Login not required OR already logged in
			...
			} else if (await (supabase.auth.getSession()).data.session) { // Alrady logged in?
				return;
				...
				let {error} = await session.auth.signInWithPassword({
					email: env.SUPABASE_USER,
					password: env.SUPABASE_PASS,
				});
				if (error) supabase.throw(error);
			}
		}) // }}}
		.then(()=> session)
}
...
supabasey.middleware = function supabaseyMiddleware(options) {
	...
	*/
	return async (req, res, env, next) => {
		return supabasey.init(env, options)
			.then(session => {
				if (settings.injectEnv) env.supabase = session;
			})
			.then(()=> next()) // Continue middleware chain
			.then(err => supabasey.throw(err))
	};
}
